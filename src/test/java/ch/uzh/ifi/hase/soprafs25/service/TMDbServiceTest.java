...
public class TMDbServiceTest {
    ...
    @Test
    public void mapTmdbMovieToEntity_dateParsingError_handlesGracefully() throws Exception {
        // Setup - create movie JSON with invalid release date
        ObjectMapper mapper = new ObjectMapper();
        ObjectNode movieJson = mapper.createObjectNode();
        movieJson.put("id", 123);
        movieJson.put("title", "Test Movie");
        movieJson.put("release_date", "not-a-date");
        ...
        movieJson.set("genres", genresArray);

        // Test - method should handle the date parsing exception
        // You may need to create a mock for your mapTmdbMovieToEntity method's parameters
        // This is a simplified example
    }

    @Test
    public void mapTmdbMovieToEntity_unknownGenre_usesUnknownLabel() throws Exception {
        // Setup - create movie JSON with unknown genre ID
        ObjectMapper mapper = new ObjectMapper();
        ObjectNode movieJson = mapper.createObjectNode();
        movieJson.put("id", 123);
        movieJson.put("title", "Test Movie");

        // Setup genre with an ID that doesn't exist in the map
        ArrayNode genresArray = mapper.createArrayNode();
        ObjectNode genre = mapper.createObjectNode();
        genre.put("id", 9999);  // Unknown ID
        genresArray.add(genre);
        movieJson.set("genres", genresArray);

        // Test - should map the unknown genre ID to "Unknown"
        // You may need to create a method accessor or modify your test approach based on how your code is structured
    }
}